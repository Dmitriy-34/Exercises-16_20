...
# свободного падения равно 9,8 м/с2. При известных начальной скорости (vi), ускорении (g) и дистанции (d) можно вычислить скорость 
# при соприкосновении объекта с землей по формуле: vf = корень из vi**2 + 2gd 

from math import sqrt

vi = 0     # Определяем константы
g = 9.8

d = float(input('Высота отпускания объекта (м): '))  # Запрашиваем высоту

vf = sqrt(2 * g * d)   # Производим рассчёт, исключая из формулы ускорение, т.к по условию оно равно 0

print(f'"Объект достигнет земли на скорости %.1f м/с' % vf)  # Выводим результат
...
R = 8.314      # Вводим константу газовой постоянной

P = float(input('Введите значение давления (в Паскалях): '))    # Запрашиваем данные от пользователя
V = float(input('Объём (в литрах): '))
T = float(input('Температура (в Кельвинах): '))

n = P*V / (R*T)     # Производим вычисление

print('Количество газа %.1f моля(молей)' % n)   # Результат программы 

# Проверка программы 1/ температура предоставлена в Цельсиях
...
print('Количество вещества %.1f моль' % n)

# Проверка программы 2/ температура предоставлена в Фаренгейтах
